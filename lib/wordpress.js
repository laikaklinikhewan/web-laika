...
export async function getOnePageContent(context) {
  ...
  if (resData.post.mediaEmbed.mediaUrl) {
    if (resData.post.mediaEmbed.mediaSource.toLowerCase() === "youtube") {
      oembed = await fetch(
        `https://youtube.com/oembed?url=${resData.post.mediaEmbed.mediaUrl}`
      );
      oembedJSON = await oembed.json();
    } else if (
      resData.post.mediaEmbed.mediaSource.toLowerCase() === "spotify"
    ) {
      oembed = await fetch(
        `https://embed.spotify.com/oembed/?url=${resData.post.mediaEmbed.mediaUrl}`
      );
    }
  }
